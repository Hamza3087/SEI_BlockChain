...
import {menu,secoundmenu} from "./SubscriptionMenuData";
import { NavLink, Link } from "react-router-dom";
import Icon from "../../components/icon/Icon";
import classNames from "classnames";

const MenuHeading = ({ heading, alt }) => {
  return (
    <li className="nk-menu-heading">
      {alt ? <span>{heading}</span> :<h6 className="overline-title text-primary-alt">{heading}</h6>}
    </li>
  );
};

const MenuItem = ({ icon, link, text, sub, subPanel, panel, sidebarToggle, count, newTab, mobileView, ...props }) => {
  let currentUrl;
    ...
    <li className={menuItemClass} onClick={(e) => toggleActionSidebar(e)}>
      {newTab ? (
        ...
        >
          ...
          <span className="nk-menu-text">{text}{count && <span className="count">({count})</span>}</span>
        </Link>
      ) : (
        <NavLink
          to={`${process.env.PUBLIC_URL + link}`}
...
const MenuSub = ({ icon, link, text, sub, sidebarToggle, count, mobileView, ...props }) => {
  return (
  ...
  return (
    <>
      ...
      <ul className="nk-menu nk-menu-sm">
        {secoundmenu.map((item, index) =>
          item.heading ? (
            <MenuHeading alt heading={item.heading} key={item.heading} />
          ) : (
            <MenuItem
              key={item.text}
              link={item.link}
